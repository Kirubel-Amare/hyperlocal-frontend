...
export default function CustomerJobsPage() {
 ...
 : 'bg-white /60 backdrop-blur-md text-gray-500 dark:text-gray-400 hover:bg-white dark:bg-gray-950 hover:text-gray-900 dark:text-gray-100 hover:shadow-md'
                        ...
                        >
                            <Icon size={16} />
                            {tab.label}
                            <span className={`px-2 py-0.5 rounded-full text-[10px] ml-1 ${isActive ? 'bg-white dark:bg-gray-950/20 text-white' : 'bg-gray-100 text-gray-500 dark:text-gray-400'}`}>
                                {tab.count}
                            </span>
                        </button>
                    )
            ...
            <div className="space-y-6">
                    ...
                    ongoingServices.map((service: any) => (
                        <div key={service.id} className="bg-white dark:bg-gray-950/60 backdrop-blur-md rounded-[32px] p-6 shadow-xl shadow-gray-200/10 border border-white dark:border-gray-800 flex flex-col md:flex-row gap-6 hover:shadow-2xl transition-all duration-500 group">
                            ...
                            <div className="flex flex-col items-center md:w-48 shrink-0 text-center gap-3 pr-6 md:border-r border-gray-100 dark:border-gray-800">
                                ...
                                <div>
                                    <h4 className="font-black text-gray-900 dark:text-gray-100">{service.provider.name}</h4>
                                    <div className="flex items-center justify-center gap-1 text-xs font-bold text-gray-500 dark:text-gray-400">
                                        <Star size={12} className="text-amber-400 fill-amber-400" />
                                        4.9
                                    </div>
                                </div>
                                <button className="px-4 py-2 bg-gray-50 dark:bg-gray-900 hover:bg-[#E8F4F4] text-[#1E7B7C] rounded-xl text-xs font-bold w-full transition-colors">
                                    {t('jobs.message')}
                                </button>
                            </div>
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <span className="text-xs font-bold text-gray-400 dark:text-gray-500">{t('jobs.jobId', { id: service.id })}</span>
                                        </div>
                                        <h3 className="text-xl font-black text-gray-900 dark:text-gray-100">{service.title}</h3>
                                    </div>
                                    <button className="p-2 hover:bg-gray-100 rounded-xl transition-colors text-gray-400 dark:text-gray-500">
                                        <MoreHorizontal size={20} />
                                    </button>
                                </div>

                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 bg-gray-50/50 dark:bg-gray-900/50 p-4 rounded-2xl">
                                    <div>
                                        <span className="text-xs font-bold text-gray-400 dark:text-gray-500 block mb-1">{t('jobs.dateTime')}</span>
                                        <span className="text-sm font-black text-gray-900 dark:text-gray-100">{service.date}</span>
                                    </div>
                                    <div>
                                        <span className="text-xs font-bold text-gray-400 dark:text-gray-500 block mb-1">{t('jobs.agreedPrice')}</span>
                                        <span className="text-sm font-black text-gray-900 dark:text-gray-100">{service.amount}</span>
                                    </div>
                                    <div>
                                        <span className="text-xs font-bold text-gray-400 dark:text-gray-500 block mb-1">{t('jobs.location')}</span>
                                        <span className="text-sm font-black text-gray-900 dark:text-gray-100 line-clamp-1" title={service.location}>{service.location}</span>
                                    </div>
                                </div>
                ...
                {activeTab === 'pending' && (
                    <div className="bg-white dark:bg-gray-950/60 backdrop-blur-md rounded-[32px] p-12 shadow-xl shadow-gray-200/10 border border-white dark:border-gray-800 flex flex-col items-center justify-center text-center">
                        ...
                        <h3 className="text-2xl font-black text-gray-900 dark:text-gray-100 mb-2">{t('jobs.pendingTitle', { count: 1 })}</h3>
                        ...
                        <div key={service.id} className="bg-white dark:bg-gray-950/60 backdrop-blur-md rounded-[32px] p-6 shadow-xl shadow-gray-200/10 border border-white dark:border-gray-800 flex flex-col md:flex-row items-center gap-6 opacity-80 hover:opacity-100 transition-opacity duration-300">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    <h3 className="text-lg font-black text-gray-900 dark:text-gray-100 truncate">{service.title}</h3>
                            ...
                            <div className="flex flex-col items-end gap-2 shrink-0">
                                <span className="text-lg font-black text-gray-900 dark:text-gray-100">{service.amount}</span>
                            ...
                            <div className="flex items-center gap-2 pl-4 border-l border-gray-100 dark:border-gray-800 shrink-0">
                                <button className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 dark:text-gray-300 rounded-xl font-bold text-xs transition-colors">
                                    {t('jobs.receipt')}
                                </button>
