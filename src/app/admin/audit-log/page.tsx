    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight mb-2">Administrative Audit Log</h1>
                    <p className="text-gray-500 font-medium">Immutable record of all administrative actions and system modifications.</p>
                </div>
            ...
            <div className="bg-white/70 backdrop-blur-xl border border-gray-100 rounded-[2.5rem] shadow-sm overflow-hidden">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex p-1 bg-gray-50 rounded-2xl border border-gray-100">
                            {['All', 'Success', 'Warning', 'Failure'].map((filter) => (
                                <button key={filter} className={`px-4 py-1.5 rounded-xl text-xs font-black uppercase transition-all ${filter === 'All' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}>
                                    {filter}
                                </button>
                            ))}
                        </div>
                    ...
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50">
                                <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Event ID</th>
                                <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Administrator</th>
                                <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Log Detail</th>
                                ...
                                <th className="px-6 py-4 text-right text-[10px] font-black text-gray-400 uppercase tracking-widest">Timestamp</th>
                            </tr>
                        </thead>
                            ...
                            {auditEntries.map((log) => (
                                <tr key={log.id} className="group hover:bg-gray-50/30 transition-colors">
                                    <td className="px-6 py-5">
                                        <span className="text-xs font-black text-gray-900 mono">{log.id}</span>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-sm font-bold text-gray-700">{log.admin}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-2">
                                    ...
                                    <td className="px-6 py-5">
                                        <span className="text-[10px] font-black uppercase px-2 py-1 bg-gray-100 text-gray-500 rounded-md tracking-tight">
                                            {log.category}
                                        </span>
                                    </td>
                                    <td className="px-6 py-5">
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                        Total Records: <span className="text-gray-900">42,891</span>
                    </p>
