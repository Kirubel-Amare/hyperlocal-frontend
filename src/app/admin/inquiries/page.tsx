    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight mb-2 italic">Guest Inquiries</h1>
                    <p className="text-gray-500 font-medium italic">Manage messages and support requests from guests and non-registered visitors.</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
                <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-gray-900">{inquiryList.filter(i => i.status === 'New').length}</h3>
                </div>
                <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm">
                ...
                <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-gray-900">{inquiryList.filter(i => i.status === 'Resolved').length}</h3>
                </div>
                <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-gray-900">{inquiryList.filter(i => i.status === 'Spam').length}</h3>
                </div>
            </div>
                                ...
                                <tr key={inq.id} className="group hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-black text-gray-900 group-hover:text-[#1E7B7C] transition-colors italic">{inq.name}</span>
                                            <span className="text-xs text-gray-500 font-bold italic">{inq.email}</span>
                                            <span className="text-[10px] font-black text-indigo-500 mt-1 uppercase tracking-widest italic">{inq.subject}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <p className="text-xs text-gray-500 font-medium line-clamp-1 max-w-[300px] italic">
                                            {inq.message}
                                        </p>
                                    </td>
                                    <td className="px-6 py-5">
                                    ...
                                    <td className="px-6 py-5">
                                        <span className="text-sm font-bold text-gray-500 italic">{inq.date}</span>
                                    </td>
                                    <td className="px-6 py-5 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <button
                                                ...
                                                title="Reply"
                                                className="p-2 text-gray-400 hover:text-blue-500 hover:bg-white rounded-xl shadow-sm transition-all border border-transparent hover:border-gray-100"
                                            >
                                                <Reply size={18} />
                                            </button>
                                            <button
