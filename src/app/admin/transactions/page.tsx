  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight mb-2 italic">Financial Operations</h1>
          <p className="text-gray-500 font-medium italic">Monitor payments, platform fees, and refund activities.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-gray-100 p-6 rounded-[2.5rem] shadow-sm group hover:shadow-xl transition-all duration-300">
          ...
          <p className="text-sm font-bold text-gray-400 mb-1 italic">Total Volume</p>
          <h3 className="text-2xl font-black text-gray-900 italic">${totalVolume.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
        </div>
        <div className="bg-white border border-gray-100 p-6 rounded-[2.5rem] shadow-sm group hover:shadow-xl transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-sm font-bold text-gray-400 mb-1 italic">Platform Revenue</p>
          <h3 className="text-2xl font-black text-gray-900 italic">${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
        </div>
        <div className="bg-white border border-gray-100 p-6 rounded-[2.5rem] shadow-sm group hover:shadow-xl transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-sm font-bold text-gray-400 mb-1 italic">Pending Payouts</p>
          <h3 className="text-2xl font-black text-gray-900 italic">${totalPending.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
        </div>
      </div>
      ...
      <div className="bg-white/70 backdrop-blur-xl border border-gray-100 rounded-[2.5rem] shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            <div className="relative group">
              ...
              <input
                ...
                className="pl-11 pr-4 py-2.5 bg-gray-50 border border-transparent rounded-2xl outline-none focus:bg-white focus:border-[#1E7B7C]/20 focus:ring-4 focus:ring-[#1E7B7C]/5 transition-all text-sm font-medium w-64 shadow-sm italic"
              />
            </div>
            <button className="p-2.5 bg-white border border-gray-100 rounded-xl text-gray-400 hover:text-[#1E7B7C] shadow-sm italic">
              <Filter size={18} />
            </button>
          </div>
        </div>
              ...
              {filteredTransactions.length > 0 ? filteredTransactions.map((trx) => (
                <tr key={trx.id} className="group hover:bg-gray-50/50 transition-all duration-300">
                  <td className="px-6 py-5">
                    <span className="text-sm font-black text-gray-900 group-hover:text-[#1E7B7C] transition-colors italic">{trx.id}</span>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-gray-400 italic">From:</span>
                        <span className="text-sm font-bold text-gray-700 italic">{trx.customer}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-gray-400 italic">To:</span>
                  ...
                  <td className="px-6 py-5">
                    <div>
                      <p className="text-sm font-black text-gray-900 italic">${trx.amount.toFixed(2)}</p>
                  ...
                  <td className="px-6 py-5">
                    <p className="text-sm font-bold text-gray-500 italic">{trx.date}</p>
                  </td>
        ...
        <div className="p-6 border-t border-gray-100 flex items-center justify-between">
          <p className="text-sm font-bold text-gray-400 italic">
            Showing <span className="text-gray-900 font-black italic">1 - {filteredTransactions.length}</span> of <span className="text-gray-900 font-black italic">{trxList.length}</span> transactions
          </p>
          <div className="flex items-center gap-2">
