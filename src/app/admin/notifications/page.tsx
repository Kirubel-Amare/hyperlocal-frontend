    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight mb-2 italic">System Notifications</h1>
                    <p className="text-gray-500 font-medium italic">Broadcast alerts, announcements, and system updates to different user groups.</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-sm font-bold text-gray-400 mb-0.5 italic">Total Broadcasts</p>
                        <h3 className="text-2xl font-black text-gray-900">{notifList.length}</h3>
                    </div>
                </div>
                <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-sm font-bold text-gray-400 mb-0.5 italic">Avg. Engagement</p>
                ...
                <div className="bg-white border border-gray-100 p-6 rounded-[2rem] shadow-sm flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-sm font-bold text-gray-400 mb-0.5 italic">Active Drafts</p>
                        <h3 className="text-2xl font-black text-gray-900">{notifList.filter(n => n.status === 'Draft').length}</h3>
                    </div>
                </div>
            </div>
                                ...
                                <tr key={not.id} className="group hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col max-w-[300px]">
                                            <span className="text-sm font-black text-gray-900 group-hover:text-[#1E7B7C] transition-colors italic">{not.title}</span>
                                            <p className="text-xs text-gray-500 font-medium line-clamp-1 italic">{not.message}</p>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-gray-700 italic">{not.target}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 ml-1 italic">{not.type}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-2">
                                    ...
                                    <td className="px-6 py-5">
                                        <span className="text-sm font-bold text-gray-500 italic">{not.date}</span>
                                    </td>
                                    <td className="px-6 py-5 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button
                                                ...
                                                className={`p-2 rounded-xl shadow-sm transition-all border border-transparent hover:border-gray-100 ${not.status === 'Sent' ? 'text-gray-400 hover:text-[#1E7B7C]' : 'text-emerald-500 hover:bg-emerald-50'}`}
                                            >
                                                <Megaphone size={18} />
                                            </button>
