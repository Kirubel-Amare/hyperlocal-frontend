    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight mb-2 italic">Job Management</h1>
                    <p className="text-gray-500 dark:text-gray-400 font-medium italic">Review, track and manage all service requests across the platform.</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-6">
                <div className="bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 p-6 rounded-[2rem] shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-gray-900 dark:text-gray-100">{jobList.length}</h3>
                </div>
                <div className="bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 p-6 rounded-[2rem] shadow-sm">
                    <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 underline decoration-emerald-500/30 italic">Active</p>
                    <h3 className="text-2xl font-black text-gray-900 dark:text-gray-100">{jobList.filter(j => j.status === 'In Progress').length}</h3>
                </div>
                <div className="bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 p-6 rounded-[2rem] shadow-sm">
                    <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 underline decoration-indigo-500/30 italic">Escrow Held</p>
                    <h3 className="text-2xl font-black text-indigo-600">${totalEscrow.toFixed(2)}</h3>
                </div>
                <div className="bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 p-6 rounded-[2rem] shadow-sm">
                    <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 underline decoration-amber-500/30 italic">Pending</p>
                    <h3 className="text-2xl font-black text-gray-900 dark:text-gray-100">{jobList.filter(j => j.status === 'Pending').length}</h3>
                </div>
                <div className="bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 p-6 rounded-[2rem] shadow-sm">
                    <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 underline decoration-red-500/30 italic">Cancelled</p>
                    <h3 className="text-2xl font-black text-gray-900 dark:text-gray-100">{jobList.filter(j => j.status === 'Cancelled').length}</h3>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-950/70 backdrop-blur-xl border border-gray-100 dark:border-gray-800 rounded-[2.5rem] shadow-sm overflow-hidden">
                    ...
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50 dark:bg-gray-900/50">
                                <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Job Details</th>
                                <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Participants</th>
                                <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Status & Priority</th>
                                <th className="px-6 py-4 text-left text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Location & Date</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredJobs.length > 0 ? filteredJobs.map((job) => (
                                <tr key={job.id} className="group hover:bg-gray-50/50 dark:bg-gray-900/50 transition-colors">
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-black text-gray-900 dark:text-gray-100 group-hover:text-[#1E7B7C] transition-colors italic">{job.id}</span>
                                            <span className="text-xs text-gray-500 dark:text-gray-400 font-bold italic">{job.service}</span>
                                    ...
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col gap-1">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-bold text-gray-700 dark:text-gray-300 italic">{job.customer}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <div className="w-1.5 h-1.5 rounded-full bg-[#1E7B7C]" />
                                                <span className="text-xs font-bold text-gray-400 dark:text-gray-500 italic">{job.provider}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <span className={`text-[9px] font-black uppercase tracking-widest ml-1 italic ${job.priority === 'High' ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'
                                                }`}>
                                    ...
                                    <td className="px-6 py-5 text-right">
                                        <div className="flex items-center justify-end gap-2 group-hover:opacity-100 transition-opacity">
                                            <button title="View Details" className="p-2 text-gray-400 dark:text-gray-500 hover:text-[#1E7B7C] hover:bg-white dark:bg-gray-950 rounded-xl shadow-sm transition-all border border-transparent hover:border-gray-100 dark:border-gray-800">
                                                <Eye size={18} />
                                            </button>
                                            <button
                                                onClick={() => handleAction(job.id, 'reassign')}
                                                title="Reassign Provider"
                                                className="p-2 text-gray-400 dark:text-gray-500 hover:text-indigo-500 hover:bg-white dark:bg-gray-950 rounded-xl shadow-sm transition-all border border-transparent hover:border-gray-100 dark:border-gray-800"
                                            >
                                                <UserPlus size={18} />
                                            </button>
                                            <button
                                            ...
                                            <button
                                                ...
                                                className="p-2 text-gray-400 dark:text-gray-500 hover:text-amber-500 hover:bg-white dark:bg-gray-950 rounded-xl shadow-sm transition-all border border-transparent hover:border-gray-100 dark:border-gray-800"
                                            >
                                                <Flag size={18} />
                                            </button>
                                        </div>
                ...
                <div className="p-6 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
                    <p className="text-sm font-bold text-gray-400 dark:text-gray-500 italic">
                        Showing <span className="text-gray-900 dark:text-gray-100 font-black italic">1 - {filteredJobs.length}</span> of <span className="text-gray-900 dark:text-gray-100 font-black italic">{jobList.length}</span> jobs
                    </p>
                    <div className="flex items-center gap-2">
            ...
            <div className="bg-amber-50 border border-amber-100 p-6 rounded-[2rem] flex items-start gap-4">
                <div className="p-3 bg-white dark:bg-gray-950 rounded-2xl text-amber-500 shadow-sm">
                    <AlertCircle size={24} />
                </div>
                <div>
